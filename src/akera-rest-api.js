...
function AkeraRestApi(akeraWebApp) {
   ...
   this.handleRequest = function(req, res) {
      ...
      if (req.method === 'GET') {
         ...
         callProc = req.body.procedure || req.body.call
            && req.body.call.procedure;
               ...
               try {
                  ...
                  if (callParams instanceof Array) {
                     ...
                     callParams.forEach(function(param) {
                        ...
                        switch (param.type) {
                           case 'inout':
                              parameters.push(p.inout(param.value,
                                 param.dataType));
                              break;
                           case 'output':
                              ...
                              break;
                           default:
                              parameters.push(p.input(param.value,
                                 param.dataType));
                        }
                     });
   ...
   if (akeraWebApp !== undefined) {
      throw new Error(
         'Rest API service can only be mounted at the broker level.');
   }
}
