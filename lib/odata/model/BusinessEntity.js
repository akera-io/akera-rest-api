...
const akeraApi = require("@akeraio/api");
class BusinessEntity {
    read(query, broker, cb) {
        const mdlInfo = this.util.getEntityNameFromRoot(query.collection, query.model);
        if (!mdlInfo)
        ...
        akeraApi.connect(broker).then(function (conn) {
            const q = this.util.transformAkeraQuery(new akeraApi.QuerySelect(table, namespace), akeraApi.Query, akeraApi.Query.arguments, akeraApi.Query.name);
            q.build();
            q.all().then(function (result) {
                let entityType;
                try {
                ...
                result.forEach(function (r) {
                    this.prune(r, query.model.getNamespace(mdlInfo.namespace), entityType);
                });
                conn.disconnect();
                cb(null, result);
            }, function (err) {
    ...
    prune(doc, model, type) {
        ...
        for (let prop in doc) {
            ...
            if (propDef.type.indexOf("Edm") !== 0) {
                ...
                const complexType = model.complexTypes && model.complexTypes[complexTypeName];
                if (complexType)
                    this.prune(doc[prop], model, complexType);
            }
        }
    }
}
